...
The live tests assert fullscreen geometry, flat Smoke background, missing toolbar, zero page overflow, keyboard list/reset paths, callout entrance/exit/Read more motion and its reduced-motion absence, the missing connection count, callout clamping, library failure, and JavaScript-disabled fallbacks in Chrome.
