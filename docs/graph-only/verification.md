...
- **No-JavaScript fallback:** at desktop and phone sizes, the interactive stage is absent and the complete 52-node, 208-connection-link index fills the viewport and scrolls internally. The skip link, readable profile content, and Connect actions remain available.
...
The live tests assert fullscreen geometry, flat background, missing toolbar, zero page overflow, keyboard list/reset paths, reduced motion, callout clamping, library failure, and JavaScript-disabled fallbacks. They fail on regressions and close their temporary Chrome and server processes in `finally` blocks.
