...
At 390 px there was no horizontal overflow, with an aspect-ratio-reserved 358 × 447.5 CSS px phone stage. On keyboard, Tab reaches an index button; Enter opens the node panel, focus moves into it, Escape closes it and returns focus to the originating button. Canvas is hidden from accessibility APIs; the complete DOM index is still in the tree. For current node and edge counts see `src/constant/graph.js`; the generated index and connection links are in `index.html`. The real `prefers-reduced-motion: reduce` media query was tested in Chrome using `--force-prefers-reduced-motion`: the graph appeared settled, with the manual control reading "Resume motion"; selecting a node still highlights its neighbourhood. The library-failure screenshot was taken with a server returning 503 for the vendor script: no canvas, controls hidden, the complete index visible. With JavaScript disabled in Chrome, the readable index and its connection links remained visible while the stage and controls were hidden; there was no blank graph slot. `scripts/test-hero-live.mjs` checks this browser path. If the main module fails, a load guard promotes the static index.
