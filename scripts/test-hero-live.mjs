...
try {
  ...
  async function go(path='/', js=true, width=1440, height=900, reduced=false) {
    ...
    for (let i=0;i<100;i++) { if (await evalJs(`!!document.querySelector('#graph-index-wrap') && (!${js} || document.documentElement.dataset.graphReady === 'true' || document.querySelector('#graph-status')?.textContent.includes('unavailable'))`)) break; await delay(200); }
    await delay(500);
  }
  ...
  await go();
  await check('desktop first paint, decluttered shell, content and SEO', `(()=>{const stage=document.querySelector('#graph-stage'),status=document.querySelector('#graph-status');return {canvas:!!document.querySelector('#graph-canvas-host canvas'),status:status.textContent,statusRole:status.getAttribute('role'),statusHidden:getComputedStyle(status).position==='absolute'&&getComputedStyle(status).clip!=='auto',caption:!!document.querySelector('#graph-caption'),description:stage.getAttribute('aria-describedby'),instructions:document.querySelector('#graph-instructions')?.textContent,stageBorder:getComputedStyle(stage).borderWidth,listBorder:getComputedStyle(document.querySelector('#graph-index-wrap')).borderWidth,panelBorder:getComputedStyle(document.querySelector('#graph-panel')).borderWidth,buttons:document.querySelector('.graph-buttons').getBoundingClientRect().toJSON(),index:document.querySelectorAll('#graph-index [data-node]').length,links:document.querySelectorAll('#graph-index .graph-index-connections a').length,graphOnly:!document.querySelector('header,nav,footer,section'),fits:document.documentElement.scrollHeight<=innerHeight&&document.documentElement.scrollWidth<=innerWidth,cta:!!document.querySelector('a[href="./assets/abdelrhmanehab_resume.pdf"][download]'),title:document.title,ld:JSON.parse(document.querySelector('script[type="application/ld+json"]').textContent)['@type'],og:document.querySelector('meta[property="og:title"]')?.content,canonical:document.querySelector('link[rel="canonical"]')?.href,bg:getComputedStyle(stage).backgroundColor}})()`, r=>r.canvas&&r.status==='Explore the profile graph'&&r.statusRole==='status'&&r.statusHidden&&!r.caption&&r.description==='graph-instructions'&&r.instructions.includes('pinch to zoom')&&r.instructions.includes('Tab to explore')&&r.stageBorder==='0px'&&r.panelBorder==='1px'&&r.buttons.left>1000&&r.index===52&&r.links===208&&r.graphOnly&&r.fits&&r.cta&&r.ld==='Person'&&r.og&&r.canonical);
  ...
  await check('List view exposes the index', `({visible:getComputedStyle(document.querySelector('#graph-index-wrap')).clip === 'auto',expanded:document.querySelector('#btn-list').getAttribute('aria-expanded'),entries:document.querySelectorAll('#graph-index [data-node]').length,border:getComputedStyle(document.querySelector('#graph-index-wrap')).borderWidth,hint:document.querySelector('.graph-index-interactive-hint').innerText})`, r=>r.visible&&r.expanded==='true'&&r.entries===52&&r.border==='1px'&&r.hint.includes('Enter'));
  ...
  await check('mobile has all nodes, no caption or count, and preserved controls position', `(()=>{const s=document.querySelector('#graph-stage'),c=document.querySelector('#graph-controls'),b=document.querySelector('.graph-buttons');return {stage:s.getBoundingClientRect().toJSON(),controls:c.getBoundingClientRect().toJSON(),buttons:b.getBoundingClientRect().toJSON(),canvas:!!document.querySelector('canvas'),status:document.querySelector('#graph-status').textContent,caption:!!document.querySelector('#graph-caption'),border:getComputedStyle(s).borderWidth,list:document.querySelectorAll('#graph-index [data-node]').length,overflow:document.documentElement.scrollWidth>innerWidth||document.documentElement.scrollHeight>innerHeight}})()`,r=>r.canvas&&r.list===52&&!r.overflow&&r.stage.width===358&&r.stage.top>90&&r.buttons.top>35&&r.status==='Explore the profile graph'&&!r.caption&&r.border==='0px');
  ...
  await check('reduced motion settled graph', `({pressed:document.querySelector('#btn-motion').getAttribute('aria-pressed'),canvas:!!document.querySelector('canvas'),status:document.querySelector('#graph-status').textContent})`,r=>r.pressed==='true'&&r.canvas&&r.status==='Explore the profile graph'&&!/\d+\s+nodes\s*·\s*\d+\s+connections/.test(r.status));
  ...
  await check('vendor failure deep link visible and status remains accessible', `({stage:document.querySelector('#graph-stage').hidden,index:getComputedStyle(document.querySelector('#graph-index-wrap')).display,panel:document.querySelector('#graph-panel').textContent,title:document.querySelector('#graph-panel-title')?.textContent,hidden:document.querySelector('#graph-panel').hidden,status:document.querySelector('#graph-status').textContent,statusRole:document.querySelector('#graph-status').getAttribute('role'),statusHidden:getComputedStyle(document.querySelector('#graph-status')).position==='absolute',hint:getComputedStyle(document.querySelector('.graph-index-interactive-hint')).display})`,r=>r.stage&&r.title==='EFA'&&!r.hidden&&r.status==='Graph unavailable — showing profile list'&&r.statusRole==='status'&&r.statusHidden&&r.hint==='none');
  ...
  failVendor=false; await go('/',false,1440,900);
  await check('desktop without JavaScript has the uncluttered static index', `({html:document.documentElement.className,stage:getComputedStyle(document.querySelector('#graph-stage')).display,index:getComputedStyle(document.querySelector('#graph-index-wrap')).display,caption:!!document.querySelector('#graph-caption'),stageBorder:getComputedStyle(document.querySelector('#graph-stage')).borderWidth,indexHints:document.querySelector('#graph-index-wrap > p').innerText,entries:document.querySelectorAll('#graph-index [data-node]').length,links:document.querySelectorAll('#graph-index .graph-index-connections a').length,first:document.querySelector('#graph-index').innerText.slice(0,120)})`,r=>r.stage==='none'&&r.index!=='none'&&!r.caption&&r.stageBorder==='0px'&&r.indexHints.includes('Tab')&&!r.indexHints.includes('Enter')&&r.entries===52&&r.links===208&&r.first.includes('Abdelrhman'));
  await screenshot('live-no-javascript-desktop.png');
  await go('/',false,390,844);
  await check('phone without JavaScript has the uncluttered static index', `({viewport:innerWidth,stage:getComputedStyle(document.querySelector('#graph-stage')).display,index:getComputedStyle(document.querySelector('#graph-index-wrap')).display,caption:!!document.querySelector('#graph-caption'),status:document.querySelector('#graph-status').textContent,hint:getComputedStyle(document.querySelector('.graph-index-interactive-hint')).display,entries:document.querySelectorAll('#graph-index [data-node]').length,links:document.querySelectorAll('#graph-index .graph-index-connections a').length,first:document.querySelector('#graph-index').innerText.slice(0,120)})`,r=>r.viewport===390&&r.stage==='none'&&r.index!=='none'&&!r.caption&&r.hint==='none'&&r.entries===52&&r.links===208&&r.first.includes('Abdelrhman'));
  await screenshot('live-no-javascript-phone.png');
