...
try {
  ...
  await check('Read more fades in the folded content with compositor-only motion', `(()=>{document.querySelector('#graph-panel-more').click();const b=document.querySelector('#graph-panel-body'),a=[...b.children].flatMap(e=>e.getAnimations());return {expanded:document.querySelector('#graph-panel-more').getAttribute('aria-expanded'),clamped:b.classList.contains('is-clamped'),count:a.length,props:a.flatMap(x=>x.effect.getKeyframes().flatMap(k=>Object.keys(k).filter(n=>!['offset','easing','composite','computedOffset'].includes(n))))}})()`, r => r.expanded === 'true' && !r.clamped && r.count > 0 && r.props.every(n => n === 'opacity' || n === 'transform'));
  await delay(400);
