...
try {
  ...
  await check('Escape starts the callout exit at once: inert and fading while focus returns', `(()=>{const p=document.querySelector('#graph-panel');return {hidden:p.hidden,inert:p.inert,animations:p.getAnimations().length,node:document.activeElement.dataset.node,hash:location.hash}})()`, r => !r.hidden && r.inert && r.animations > 0 && r.node === 'me' && r.hash === '');
  await delay(350);
  await check('Escape closes detail and restores the list entry focus', `({hidden:document.querySelector('#graph-panel').hidden,inert:document.querySelector('#graph-panel').inert,node:document.activeElement.dataset.node})`, r => r.hidden && !r.inert && r.node === 'me');
  await key('Escape', 'Escape', 27);
