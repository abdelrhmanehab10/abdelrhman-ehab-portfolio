...
try {
  ...
  await go();
  await check('desktop first paint, content and SEO', `({canvas:!!document.querySelector('#graph-canvas-host canvas'),status:document.querySelector('#graph-status').textContent,index:document.querySelectorAll('#graph-index [data-node]').length,links:document.querySelectorAll('#graph-index .graph-index-connections a').length,hero:document.querySelector('#hero-meta').textContent,cta:!!document.querySelector('a[href="./assets/abdelrhmanehab_resume.pdf"]'),title:document.title,ld:JSON.parse(document.querySelector('script[type="application/ld+json"]').textContent)['@type'],og:document.querySelector('meta[property="og:title"]')?.content,canonical:document.querySelector('link[rel="canonical"]')?.href,bg:getComputedStyle(document.querySelector('#graph-stage')).backgroundColor})`, r=>r.canvas&&r.status.includes('52 nodes')&&r.status.includes('104 connections')&&r.index===52&&r.links===208&&r.hero.includes('React')&&r.cta&&r.ld==='Person'&&r.og&&r.canonical);
  ...
  await check('mobile has all nodes and stage', `({stage:document.querySelector('#graph-stage').getBoundingClientRect().toJSON(),canvas:!!document.querySelector('canvas'),list:document.querySelectorAll('#graph-index [data-node]').length,overflow:document.documentElement.scrollWidth>innerWidth})`,r=>r.canvas&&r.list===52&&!r.overflow&&r.stage.width>300);
