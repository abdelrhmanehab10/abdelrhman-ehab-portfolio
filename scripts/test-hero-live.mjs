...
try {
  ...
  await check('List view exposes the index', `({visible:getComputedStyle(document.querySelector('#graph-index-wrap')).clip === 'auto',expanded:document.querySelector('#btn-list').getAttribute('aria-expanded'),entries:document.querySelectorAll('#graph-index [data-node]').length,border:getComputedStyle(document.querySelector('#graph-index-wrap')).borderWidth})`, r=>r.visible&&r.expanded==='true'&&r.entries===52&&r.border==='1px');
  ...
  await check('desktop without JavaScript has the uncluttered static index', `({html:document.documentElement.className,stage:getComputedStyle(document.querySelector('#graph-stage')).display,index:getComputedStyle(document.querySelector('#graph-index-wrap')).display,caption:!!document.querySelector('#graph-caption'),stageBorder:getComputedStyle(document.querySelector('#graph-stage')).borderWidth,indexHints:document.querySelector('#graph-index-wrap > p').textContent,entries:document.querySelectorAll('#graph-index [data-node]').length,links:document.querySelectorAll('#graph-index .graph-index-connections a').length,first:document.querySelector('#graph-index').innerText.slice(0,120)})`,r=>r.stage==='none'&&r.index!=='none'&&!r.caption&&r.stageBorder==='0px'&&r.indexHints.includes('Tab')&&r.indexHints.includes('Enter')&&r.entries===52&&r.links===208&&r.first.includes('Abdelrhman'));
  await screenshot('live-no-javascript-desktop.png');
  await go('/',false,390,844);
  await check('phone without JavaScript has the uncluttered static index', `({viewport:innerWidth,stage:getComputedStyle(document.querySelector('#graph-stage')).display,index:getComputedStyle(document.querySelector('#graph-index-wrap')).display,caption:!!document.querySelector('#graph-caption'),status:document.querySelector('#graph-status').textContent,entries:document.querySelectorAll('#graph-index [data-node]').length,links:document.querySelectorAll('#graph-index .graph-index-connections a').length,first:document.querySelector('#graph-index').innerText.slice(0,120)})`,r=>r.viewport===390&&r.stage==='none'&&r.index!=='none'&&!r.caption&&r.entries===52&&r.links===208&&r.first.includes('Abdelrhman'));
