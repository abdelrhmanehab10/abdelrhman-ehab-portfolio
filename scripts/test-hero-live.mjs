...
try {
  ...
  function contrastRatio(foreground, background) {
    ...
    const luminance = color => rgb(color).map(v => { const c = v / 255; return c <= 0.04045 ? c / 12.92 : ((c + 0.055) / 1.055) ** 2.4; })
  ...
  await go();
  await check('desktop fills viewport with a solid background and no chrome controls', `(()=>{const stage=document.querySelector('#graph-stage'),index=document.querySelector('#graph-index-wrap'),body=getComputedStyle(document.body),s=getComputedStyle(stage),r=stage.getBoundingClientRect();return {canvas:!!stage.querySelector('canvas'),rect:{x:r.x,y:r.y,width:r.width,height:r.height},viewport:{width:innerWidth,height:innerHeight},pageOverflow:document.documentElement.scrollWidth>innerWidth||document.documentElement.scrollHeight>innerHeight,bodyColor:body.backgroundColor,bodyImage:body.backgroundImage,stageColor:s.backgroundColor,stageImage:s.backgroundImage,stageBorder:s.borderWidth,indexBorder:getComputedStyle(index).borderWidth,controls:!!document.querySelector('#graph-controls,.graph-buttons,#btn-list,#btn-reset,#btn-motion'),caption:!!document.querySelector('#graph-caption'),panelBorder:getComputedStyle(document.querySelector('#graph-panel')).borderWidth,nodes:document.querySelectorAll('#graph-index [data-node]').length,links:document.querySelectorAll('#graph-index .graph-index-connections a').length,graphOnly:!document.querySelector('header,nav,footer,section'),status:document.querySelector('#graph-status').textContent,statusOutside:!stage.contains(document.querySelector('#graph-status')),description:stage.getAttribute('aria-describedby')}})()`, r => r.canvas && r.rect.x === 0 && r.rect.y === 0 && r.rect.width === r.viewport.width && r.rect.height === r.viewport.height && !r.pageOverflow && r.bodyColor === 'rgb(27, 27, 27)' && r.bodyImage === 'none' && r.stageColor === 'rgb(27, 27, 27)' && r.stageImage === 'none' && r.stageBorder === '0px' && r.indexBorder === '0px' && !r.controls && !r.caption && r.panelBorder === '1px' && r.nodes === 52 && r.links === 208 && r.graphOnly && r.status === 'Explore the profile graph' && r.statusOutside && r.description === 'graph-instructions');
  await check('rendered graph is neutral, visible, and readable on its page background', `(()=>{const c=document.querySelector('#graph-canvas-host canvas'),pixels=c.getContext('2d').getImageData(0,0,c.width,c.height).data,bg=getComputedStyle(document.body).backgroundColor.match(/\\d+/g).map(Number),linear=x=>{x/=255;return x<=.04045?x/12.92:((x+.055)/1.055)**2.4},lum=p=>p.map(linear).reduce((v,x,i)=>v+x*[.2126,.7152,.0722][i],0),contrast=(a,b)=>{const x=lum(a),y=lum(b);return (Math.max(x,y)+.05)/(Math.min(x,y)+.05)};let bright=0,colored=0,visible=0;for(let i=0;i<pixels.length;i+=4){const p=[...pixels.slice(i,i+3)],alpha=pixels[i+3];if(alpha<128)continue;visible++;if(Math.max(...p)-Math.min(...p)>3)colored++;const actual=p.map((v,j)=>Math.round(v*alpha/255+bg[j]*(1-alpha/255)));if(contrast(actual,bg)>=3)bright++}return {width:c.width,height:c.height,visible,bright,colored}})()`, r => r.width === 1440 && r.height === 900 && r.visible > 1000 && r.bright > 50 && r.colored === 0);
  await screenshot('desktop-default.png');
  await check('social cards load the 1200 × 630 Smoke graph preview', `(async()=>{const og=document.querySelector('meta[property="og:image"]')?.content,twitter=document.querySelector('meta[name="twitter:image"]')?.content;const image=new Image();image.src=new URL(og).pathname.replace('/abdelrhman-ehab-portfolio/','/');await image.decode();const canvas=document.createElement('canvas');canvas.width=image.width;canvas.height=image.height;const ctx=canvas.getContext('2d');ctx.drawImage(image,0,0);return {og,twitter,width:image.width,height:image.height,corner:[...ctx.getImageData(0,0,1,1).data].slice(0,3),center:[...ctx.getImageData(600,315,1,1).data].slice(0,3)}})()`, r => r.og?.endsWith('/assets/images/og-graph.png') && r.twitter === r.og && r.width === 1200 && r.height === 630 && r.corner.every(n => n === 27) && r.center.every(n => n === r.center[0]));

  await c('Page.bringToFront');
  await key('Tab', 'Tab', 9);
  await delay(250); // the skip link slides in
  ...
  await check('visible keyboard focus has contrast against the page', `(()=>{const s=getComputedStyle(document.activeElement);return {outline:s.outlineColor,background:s.backgroundColor}})()`, r => contrastRatio(r.outline,r.background) >= 3);
  ...
  await check('rendered list text contrasts with its background', `(()=>{const selectors=['#graph-index-wrap h2','.graph-index-summary','.graph-index-meta','.graph-index-link','#graph-index-return'];const bg=getComputedStyle(document.querySelector('#graph-index-wrap')).backgroundColor;return selectors.map(selector=>({selector,color:getComputedStyle(document.querySelector(selector)).color,bg}))})()`, rows => rows.every(row => contrastRatio(row.color,row.bg) >= 4.5));
  ...
  await check('rendered callout text and action have readable contrast', `(()=>{const panel=document.querySelector('#graph-panel'),bg=getComputedStyle(panel).backgroundColor,selectors=['h2','.graph-group-name','.graph-summary','.graph-facts li','#graph-panel-close'];const rows=selectors.map(selector=>({selector,color:getComputedStyle(panel.querySelector(selector)).color,bg}));const action=panel.querySelector('.graph-action');if(action){const style=getComputedStyle(action);rows.push({selector:'.graph-action',color:style.color,bg:style.backgroundColor})}return rows})()`, rows => rows.every(row => contrastRatio(row.color,row.bg) >= 4.5));
  await check('Smoke callout has no connection count and animates its entrance', `(()=>{const p=document.querySelector('#graph-panel'),s=getComputedStyle(p);return {meta:!!p.querySelector('.graph-meta'),text:p.textContent,bg:s.backgroundColor,border:s.borderTopColor,title:getComputedStyle(p.querySelector('h2')).color,animations:p.getAnimations().map(a=>a.effect.getKeyframes().flatMap(k=>Object.keys(k).filter(n=>!['offset','easing','composite','computedOffset'].includes(n))))}})()`, r => !r.meta && !/\d+\s+connections?/.test(r.text) && r.bg === 'rgb(37, 37, 37)' && r.border === 'rgb(77, 77, 77)' && r.title === 'rgb(242, 242, 242)' && r.animations.length > 0 && r.animations.flat().every(n => n === 'opacity' || n === 'transform'));
  await key('Escape', 'Escape', 27);
  const exiting = await evaluate(`(()=>{const p=document.querySelector('#graph-panel');return {hidden:p.hidden,inert:p.inert,animations:p.getAnimations().length,node:document.activeElement.dataset.node,hash:location.hash}})()`);
  await key('Escape', 'Escape', 27);
  assert.ok(!exiting.hidden && exiting.inert && exiting.animations > 0 && exiting.node === 'me' && exiting.hash === '', `Escape begins exit: ${JSON.stringify(exiting)}`);
  console.log('Escape starts callout exit', JSON.stringify(exiting));
  await check('second Escape during exit returns from the list to the graph', `({focus:document.activeElement.id,listClip:getComputedStyle(document.querySelector('#graph-index-wrap')).clip})`, r => r.focus === 'graph-stage' && r.listClip !== 'auto');
  await delay(350);
  await check('exit completes without moving focus back to the list', `({hidden:document.querySelector('#graph-panel').hidden,inert:document.querySelector('#graph-panel').inert,focus:document.activeElement.id})`, r => r.hidden && !r.inert && r.focus === 'graph-stage');
  await evaluate(`location.hash = '#node/role-pro-event'`); await delay(400);
  await check('Read more reveals folded content with compositor-friendly motion', `(()=>{document.querySelector('#graph-panel-more').click();const b=document.querySelector('#graph-panel-body'),a=[b,...b.children].flatMap(e=>e.getAnimations());return {expanded:document.querySelector('#graph-panel-more').getAttribute('aria-expanded'),clamped:b.classList.contains('is-clamped'),count:a.length,props:a.flatMap(x=>x.effect.getKeyframes().flatMap(k=>Object.keys(k).filter(n=>!['offset','easing','composite','computedOffset'].includes(n))))}})()`, r => r.expanded === 'true' && !r.clamped && r.count > 0 && r.props.every(n => ['opacity', 'transform', 'clipPath'].includes(n)));
  await evaluate(`location.hash = '#node/hub-experience'`); await delay(400);
  await check('summary-only Read more animates without fading visible lines', `(()=>{const b=document.querySelector('#graph-panel-body'),s=b.querySelector('.graph-summary'),fold=b.getBoundingClientRect().bottom,top=s.getBoundingClientRect().top,more=document.querySelector('#graph-panel-more'),crosses=top<fold&&s.getBoundingClientRect().bottom>fold;more.click();return {crosses,expanded:more.getAttribute('aria-expanded'),summaryAnimations:s.getAnimations().length,reveal:b.getAnimations().some(a=>a.effect.getKeyframes().some(k=>k.clipPath)),button:more.getAnimations().length>0}})()`, r => r.expanded === 'true' && r.summaryAnimations === 0 && r.button && (!r.crosses || r.reveal));
  await delay(400);
  await evaluate(`document.querySelector('#graph-panel-close').click()`); await delay(700);
  // The layout engine may still be cooling; the eased highlight must settle to a still canvas.
  let still = false;
  for (let i = 0; i < 12 && !still; i++) {
    const idle = await evaluate(`document.querySelector('#graph-canvas-host canvas').toDataURL()`); await delay(700);
    still = await evaluate(`document.querySelector('#graph-canvas-host canvas').toDataURL()`) === idle;
  }
  assert.ok(still, 'the eased highlight settles to a still canvas');
  console.log('highlight easing settles to a still canvas');
  ...
  await check('desktop callout retains card styling and remains inside the fullscreen stage', `(()=>{const s=document.querySelector('#graph-stage').getBoundingClientRect(),p=document.querySelector('#graph-panel'),r=p.getBoundingClientRect();return {title:document.querySelector('#graph-panel-title')?.textContent,hidden:p.hidden,inside:r.left>=s.left&&r.top>=s.top&&r.right<=s.right&&r.bottom<=s.bottom,border:getComputedStyle(p).borderWidth,stage:s.toJSON(),animations:document.getAnimations().length,meta:!!p.querySelector('.graph-meta')}})()`, r => r.title === 'Abdelrhman Ehab' && !r.hidden && r.inside && r.border === '1px' && r.animations === 0 && !r.meta);
  await screenshot('desktop-callout.png');
  await check('reduced motion closes the callout instantly without animations', `(()=>{document.querySelector('#graph-panel-more')?.click();const expanded=document.getAnimations().length;document.querySelector('#graph-panel-close').click();return {hidden:document.querySelector('#graph-panel').hidden,animations:document.getAnimations().length,expanded}})()`, r => r.hidden && r.animations === 0 && r.expanded === 0);
  await evaluate(`location.hash = '#node/link-resume'`); await delay(300);
  await check('rendered callout action contrasts with its button background', `(()=>{const action=document.querySelector('#graph-panel .graph-action');if(!action)return null;const s=getComputedStyle(action);return {color:s.color,background:s.backgroundColor,text:action.textContent.trim()}})()`, r => r && r.text && contrastRatio(r.color,r.background) >= 4.5);

  await go('/', true, 390, 844);
  await check('390px graph fills the viewport without scrollbars or decorative borders', `(()=>{const s=document.querySelector('#graph-stage'),r=s.getBoundingClientRect();return {canvas:!!s.querySelector('canvas'),rect:{x:r.x,y:r.y,width:r.width,height:r.height},viewport:{width:innerWidth,height:innerHeight},pageOverflow:document.documentElement.scrollWidth>innerWidth||document.documentElement.scrollHeight>innerHeight,body:getComputedStyle(document.body).backgroundColor,bodyImage:getComputedStyle(document.body).backgroundImage,stage:getComputedStyle(s).backgroundColor,border:getComputedStyle(s).borderWidth,indexBorder:getComputedStyle(document.querySelector('#graph-index-wrap')).borderWidth,controls:!!document.querySelector('#graph-controls,.graph-buttons,#btn-list,#btn-reset,#btn-motion'),nodes:document.querySelectorAll('#graph-index [data-node]').length}})()`, r => r.canvas && r.rect.x === 0 && r.rect.y === 0 && r.rect.width === 390 && r.rect.height === 844 && r.viewport.width === 390 && r.viewport.height === 844 && !r.pageOverflow && r.body === 'rgb(27, 27, 27)' && r.bodyImage === 'none' && r.stage === 'rgb(27, 27, 27)' && r.border === '0px' && r.indexBorder === '0px' && !r.controls && r.nodes === 52);
  await screenshot('mobile-default.png');
  await go('/#node/hub-experience', true, 280, 844);
  await check('phone summary-only Read more reveals newly visible lines without fading visible text', `(()=>{const b=document.querySelector('#graph-panel-body'),s=b.querySelector('.graph-summary'),fold=b.getBoundingClientRect().bottom,summary=s.getBoundingClientRect(),more=document.querySelector('#graph-panel-more'),crosses=summary.top<fold&&summary.bottom>fold,oldTop=document.querySelector('#graph-panel').getBoundingClientRect().top;more.click();const panel=document.querySelector('#graph-panel');return {crosses,expanded:more.getAttribute('aria-expanded'),summaryAnimations:s.getAnimations().length,reveal:b.getAnimations().some(a=>a.effect.getKeyframes().some(k=>k.clipPath)),topShift:panel.getBoundingClientRect().top-oldTop,button:more.getAnimations().length>0}})()`, r => r.crosses && r.expanded === 'true' && r.summaryAnimations === 0 && r.reveal && r.topShift === 0 && r.button);
  await screenshot('narrow-mobile-summary-reveal.png');
  await delay(400);
  await check('newly revealed summary settles without a clipping animation', `(()=>{const b=document.querySelector('#graph-panel-body'),s=b.querySelector('.graph-summary');return {summaryHeight:s.getBoundingClientRect().height,bodyHeight:b.getBoundingClientRect().height,animations:b.getAnimations().length,expanded:document.querySelector('#graph-panel-more').getAttribute('aria-expanded')}})()`, r => r.expanded === 'true' && r.summaryHeight <= r.bodyHeight && r.animations === 0);
  await screenshot('narrow-mobile-summary-expanded.png');
